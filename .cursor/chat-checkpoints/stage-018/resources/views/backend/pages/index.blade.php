        ...
        <div class="container-fluid">
                ...
                <div class="col-xl-5">
                    ...
                    <div class="tm-card">
                        <div class="tm-card-head">
                            ...
                            <a href="{{ route('manager.product.index') }}" class="small fw-semibold text-decoration-none" style="color:#0d9488;">Manage</a>
                        </div>
                        <div class="tm-card-body">
                            @forelse($lowStockList as $product)
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <div class="tm-product">
                                        <img src="{{ $product->image ? asset('storage/'.$product->image) : asset('frontend/images/no-image.png') }}" alt="">
                                        <div>
                                            <p class="name">{{ $product->name }}</p>
                                            <p class="meta">{{ $product->sku }}</p>
                                        </div>
                                    </div>
                                    <span class="badge {{ (int) $product->quantity <= 0 ? 'bg-danger' : 'bg-warning text-dark' }}">
                                        {{ (int) $product->quantity <= 0 ? 'Out' : (int) $product->quantity . ' left' }}
                                    </span>
                                </div>
                            @empty
                                <div class="tm-empty">Inventory looks healthy.</div>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-12">
                    <div class="tm-card">
                        <div class="tm-card-head">
                            <h5>Orders by Source</h5>
                        </div>
                        <div class="tm-card-body">
                            <div class="tm-chip-row">
                                @foreach($sourceBreakdown as $source => $count)
                                    <span class="tm-chip">{{ $source ?: 'Unknown' }}: <strong>{{ $count }}</strong></span>
                                @endforeach
                            </div>
                        </div>
                    </div>
