...
<body>
    ...
    <header class="header">
        ...
        <div class="header-middle">
            <div class="container">
                <div class="row align-items-center g-2 g-xl-3 flex-lg-nowrap">
                    <div class="col-5 col-lg-3 col-xl-2">
                        <div class="header-middle-logo">
                            <a class="navbar-brand" href="{{route('frontend.home.page')}}">
    @if($setting?->hasMedia('logo'))
		<img src="{{ $setting->getFirstMediaUrl('logo', 'small') ?: $setting->getFirstMediaUrl('logo') }}" alt="{{ $setting?->site_name ?? '' }}" />
	@endif
                            </a>
                        </div>
                    </div>
                    <div class="d-none d-lg-block col-lg-5 col-xl-5">
                        <div class="header-middle-search">
                            <form action="{{ route('frontend.shop') }}" method="GET" role="search">
                                <div class="search-content">
                                    ...
                                    <button type="submit" class="search-btn" aria-label="Search"><i class="far fa-search"></i></button>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="col-7 col-lg-4 col-xl-5">
                        <div class="header-middle-right">
                            <ul class="header-middle-list tm-header-actions">
                                @if(Auth::guard('web')->check())
                                    @php
                                        $authUser = Auth::guard('web')->user();
                                        $headerUnread = $authUser->unreadAppNotifications()->count();
                                        $headerNotes = $authUser->appNotifications()->latest()->take(5)->get();
                                    @endphp

                                    <li class="tm-action-item tm-dropdown">
                                        <button type="button" class="tm-action-btn tm-dropdown-toggle" aria-expanded="false" aria-label="Account menu">
                                            <span class="tm-action-icon">
                                                <i class="far fa-user"></i>
                                            </span>
                                            <span class="tm-action-meta tm-account-meta">
                                                <small>Hello</small>
                                                <strong title="{{ $authUser->name }}">{{ $authUser->name ?: 'Account' }}</strong>
                                            </span>
                                            <i class="fas fa-chevron-down tm-caret"></i>
                                        </button>
                                        <div class="tm-dropdown-panel">
                                            <div class="tm-dropdown-head">
                                                <strong>{{ $authUser->name }}</strong>
                                                <span>{{ $authUser->email }}</span>
                                            </div>
                                            <a href="{{ route('frontend.dashboard.show') }}" class="tm-dropdown-link">
                                                <i class="far fa-gauge-high"></i> Dashboard
                                            </a>
                                            <a href="{{ route('frontend.dashboard.profile') }}" class="tm-dropdown-link">
                                                <i class="far fa-user"></i> My Profile
                                            </a>
                                            <a href="{{ route('frontend.dashboard.orderlist') }}" class="tm-dropdown-link">
                                                <i class="far fa-shopping-bag"></i> My Orders
                                            </a>
                                            <a href="{{ route('frontend.notifications.index') }}" class="tm-dropdown-link">
                                                <i class="far fa-bell"></i> All Notifications
                                                @if($headerUnread > 0)
                                                    <span class="tm-inline-badge">{{ $headerUnread > 9 ? '9+' : $headerUnread }}</span>
                                                @endif
                                            </a>
                                            <div class="tm-dropdown-divider"></div>
                                            <a href="{{ route('frontend.logout') }}" class="tm-dropdown-link tm-dropdown-danger">
                                    ...
                                    <li class="tm-action-item tm-dropdown">
                                        <button type="button" class="tm-action-btn tm-dropdown-toggle" aria-expanded="false" aria-label="Notifications">
                                            <span class="tm-action-icon tm-bell-icon">
                                                <i class="far fa-bell"></i>
                                                @if($headerUnread > 0)
                                                    <span class="tm-count-badge">{{ $headerUnread > 9 ? '9+' : $headerUnread }}</span>
                                                @endif
                                            </span>
                                            <span class="tm-action-meta">
                                                <small>Alerts</small>
                                                <strong>Notifications</strong>
                                            </span>
                                        </button>
                                        <div class="tm-dropdown-panel tm-notify-panel">
                                            <div class="tm-dropdown-head">
                                                <strong>Notifications</strong>
                                                <span>{{ $headerUnread }} unread</span>
                                            </div>
                                            <div class="tm-notify-list">
                                                @forelse($headerNotes as $note)
                                                    @php
                                                        $ndata = is_array($note->data) ? $note->data : [];
                                                        $nurl = $ndata['url'] ?? route('frontend.notifications.index');
                                                    @endphp
                                                    <a href="{{ $nurl }}" class="tm-notify-item {{ $note->is_read ? '' : 'is-unread' }}">
                                                        <span class="tm-notify-dot"></span>
                                                        <span class="tm-notify-body">
                                                            <strong>{{ $note->title ?: 'Notification' }}</strong>
                                                            <em>{{ \Illuminate\Support\Str::limit($note->displayMessage(), 64) }}</em>
                                                            <small>{{ optional($note->created_at)->diffForHumans() }}</small>
                                                        </span>
                                                    </a>
                                                @empty
                                                    <div class="tm-notify-empty">No notifications yet.</div>
                                                @endforelse
                                            </div>
                                            <a href="{{ route('frontend.notifications.index') }}" class="tm-dropdown-footer-link">
                                                View all notifications
                                            </a>
                                        </div>
                                    </li>
                                @else
                                    <li class="tm-action-item">
                                        <a href="{{ route('frontend.register') }}" class="tm-action-btn">
                                            <span class="tm-action-icon">
                                ...
                                <li class="tm-action-item wishlist_count_show">
                                    @php
                                ...
                                <li class="tm-action-item dropdown-cart">
                                </li>
                            </ul>
                        </div>
                    </div>
        ...
        <div class="main-navigation">
            <nav class="navbar navbar-expand-lg">
                <div class="container position-relative">
                    <a class="navbar-brand" href="{{route('frontend.home.page')}}">
                        @if($setting?->hasMedia('logo'))
                        <img src="{{ $setting->getFirstMediaUrl('logo', 'small') ?: $setting->getFirstMediaUrl('logo') }}" class="logo-scrolled" alt="{{ $setting?->site_name ?? 'logo' }}">
                        @else
    ...
    <div class="search-popup">
        ...
        <form action="{{ route('frontend.shop') }}" method="GET" role="search">
            <div class="form-group">
                <input type="search" name="q" class="form-control" placeholder="Search products..." value="{{ request('q') }}" required>
