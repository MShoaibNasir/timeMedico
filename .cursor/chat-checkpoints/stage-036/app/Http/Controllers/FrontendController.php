...
class FrontendController extends Controller
{
    ...
    public function singleShop($id)
    {
        ...
        $product = Product::with('category', 'type_data')->where('id', $id)->firstOrFail();

        $relatedProducts = Product::with('category', 'type_data')
            ->where('status', 1)
            ->where('id', '!=', $product->id)
    ...
    {
        $id = Crypt::decryptString($id);
        return view('frontend.productFilter', [
            'id' => $id,
            'search' => request('q', ''),
        ]);
    }
    ...
    public function productlist(Request $request)
    {
        ...
        if ($request->filled('search_product')) {
            $term = trim($request->search_product);
    ...
    public function register(Request $request)
    {
        if ($request->filled('redirect')) {
            frontend_store_intended_url($request->query('redirect'));
        }
    ...
    public function signup(Request $request)
    {
        try {
            ...
            OtpDeliveryService::issueAndMail([
                'name'         => $request->name,
                'email'        => $request->email,
            ...
            $redirect = session('frontend.url.intended');

            return response()->json([
                'success' => true,
                'html' => view('frontend.otpModal', compact('phone', 'email', 'redirect'))->render()
            ]);
            ...
            $mailFailed = str_contains(strtolower($th->getMessage()), 'mail')
                || str_contains($th::class, 'Transport')
                || str_contains($th::class, 'RfcCompliance');

            return response()->json([
                'success' => false,
                'message' => $mailFailed
                    ? 'Unable to send OTP email. Please check mail settings and try again.'
    ...
    public function resendotp(Request $request)
    {
        try {
            ...
            $existing = OtpDeliveryService::latestRecord($request->otp_email, $request->otp_phone);
            if (! $existing) {
                return response()->json([
                    'success' => false,
    ...
    {
        if ($request->filled('redirect')) {
            frontend_store_intended_url($request->input('redirect'));
        }
        ...
        Auth::guard('web')->login($user);
        return redirect()->to(frontend_pull_intended_url())
            ->with(['success' => 'OTP verified and logged in successfully!']);
    }
  ...
  public function addToCart(Request $request)
    {
        ...
        // Kitni quantity already cart me hai is product ki (agar hai to)
        $existingQty = isset($cart[$product->id]) ? $cart[$product->id]['quantity'] : 0;

        // Total quantity jo cart me honi chahiye (existing + new request)
        $newTotalQty = $existingQty + $requestedQty;

        // Stock se zyada allow nahi karna
        if ($newTotalQty > $product->quantity) {
            // Jitna stock available hai utna hi allow karo
            $newTotalQty = $product->quantity;
        }

        // Agar stock hi 0 hai ya already max qty cart me add ho chuki hai
        if ($newTotalQty <= $existingQty && $existingQty >= $product->quantity) {
            return response()->json([
                'status' => false,
                'message' => 'Cannot add more items. Available stock is already in your cart.'
            ]);
        }
