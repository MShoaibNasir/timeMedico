    ...
    <div class="site-breadcrumb">
        ...
        <div class="container">
            <div class="site-breadcrumb-wrap">
                <h4 class="breadcrumb-title">{{ !empty($search) ? 'Search Results' : 'Shop' }}</h4>
                <ul class="breadcrumb-menu">
                    <li><a href="{{ route('frontend.home.page') }}"><i class="far fa-home"></i> Home</a></li>
                    <li class="active">{{ !empty($search) ? 'Search' : 'Shop' }}</li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <div class="shop-sidebar">
                        ...
                        <div class="shop-widget">
                            <div class="shop-search-form">
                                ...
                                <form action="{{ route('frontend.shop') }}" method="GET" onsubmit="return false;">
                                    <div class="form-group">
                                        <input type="text" class="form-control" placeholder="Search products..." id="search_product" value="{{ $search ?? '' }}">
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <div class="shop-widget">
                            ...
                            <ul class="shop-category-list">
                                ...
                                @foreach(($allcategories ?? $categories ?? collect()) as $cat)
                                    <li>
                                        <a href="#" class="category-filter {{ (string) $id === (string) $cat->id ? 'active' : '' }}" data-id="{{ $cat->id }}">
                                            {{ $cat->name }}
                                        </a>
                                    </li>
                                @endforeach
