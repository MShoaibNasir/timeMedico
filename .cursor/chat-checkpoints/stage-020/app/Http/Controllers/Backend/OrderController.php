...
class OrderController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$order->canTransitionTo($request->status)) {
            return redirect()->back()->with(
                'error',
                'Order status cannot move backwards. Current status is "' . $order->status . '".'
            );
        }
    ...
    {
        $serviceableAreas = Area::active()->serviceable()->orderBy('name')->get();
        $nonServiceableAreas = Area::active()->nonServiceable()->orderBy('name')->get();
        $products = Product::where('status', 1)->get();

        return view('backend.orders.placeOrder', [
    ...
    public function placeOrderStore(Request $request): RedirectResponse
    {
        ...
        if (in_array($deliveryMethod, [CartService::METHOD_LOCAL, CartService::METHOD_COURIER], true)) {
            ...
            if ($deliveryMethod === CartService::METHOD_LOCAL && ! $area->isServiceable()) {
                return back()->withInput()->with('error', 'Selected area is non-serviceable. Use Courier Service.');
            }

            if ($deliveryMethod === CartService::METHOD_COURIER && $area->isServiceable()) {
                return back()->withInput()->with('error', 'Courier Service is for non-serviceable areas only.');
            }
        ...
        $cartService = new CartService($cartItems, $area, $coupon, $deliveryMethod);
        $summary = $cartService->summary();

        $addressSnapshot = $validated['address'];
