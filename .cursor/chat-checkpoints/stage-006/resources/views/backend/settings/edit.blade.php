...
@section('content')
	...
	<div class="form-group mb-3">
		...
		<img src="{{ $setting->getFirstMediaUrl('logo', 'small') ?: $setting->getFirstMediaUrl('logo') }}" width="100" alt="logo" class="mb-2 d-block" style="max-height:100px;object-fit:contain;">
        @endif
		<input type="file" name="logo" accept=".jpg,.jpeg,.png,.svg,.webp,image/*" class="form-control">
	</div>
	
	<div class="form-group mb-3">
		<label>Favicon:</label>
		@if ($setting->hasMedia('favicon'))
		<img src="{{ $setting->getFirstMediaUrl('favicon') }}" width="32" height="32" alt="favicon" class="mb-2 d-block" style="object-fit:contain;">
        @endif
		<input type="file" name="favicon" accept=".ico,.png,.jpg,.jpeg,.webp,image/*" class="form-control">
	</div>
	...
	<div class="form-group mb-3">
		...
		<input type="text" name="google_site_verification" placeholder="Google Site Verification" class="form-control" value="{{ old('google_site_verification', $setting->google_site_verification) }}" >
	</div>
    ...
    allowClear: true
        ...
        $(document).ready(function() {
            ...
            $('#myForm').on('submit', function(e) {
                ...
                $('#hidden_code_editor').val(btoa(unescape(encodeURIComponent(codeEditorInstance.getValue()))));
                $('#hidden_js_editor').val(btoa(unescape(encodeURIComponent(javascriptEditorInstance.getValue()))));
                
                ['phone', 'address', 'copyright_text'].forEach(function(name) {
                    let field = $('[name="' + name + '"]');
                    if (field.val()) {
                        field.val(
                            btoa(unescape(encodeURIComponent(field.val())))
                        );
                    }
                });
