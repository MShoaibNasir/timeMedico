...
@endif
 ...
 <script>
     ...
     const frontendLoginBaseUrl = @json(route('frontend.login'));

     function frontendLoginUrl(returnUrl) {
         const url = new URL(frontendLoginBaseUrl, window.location.origin);
         url.searchParams.set('redirect', returnUrl || window.location.href);
         return url.toString();
     }

     function redirectToFrontendLogin(returnUrl) {
         window.location.href = frontendLoginUrl(returnUrl);
     }
