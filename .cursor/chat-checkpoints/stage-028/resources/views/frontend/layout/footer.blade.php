...
@endif
 ...
 <script>
     ...
     // Header account / notification dropdowns
     $(document).on('click', '.tm-dropdown-toggle', function (e) {
         e.preventDefault();
         e.stopPropagation();
         var $item = $(this).closest('.tm-dropdown');
         $('.tm-dropdown').not($item).removeClass('is-open');
     ...
     $(document).on('click', '.tm-dropdown-panel, .dropdown-cart-menu', function (e) {
         e.stopPropagation();
     });
     ...
     function syncProductCartAction(wrapper, quantity) {
         const qty = parseInt(quantity, 10) || 0;
         const btn = wrapper.find('.product-cart-btn');
         const qtyBox = wrapper.find('.product-card-qty');
         const qtyInput = wrapper.find('.product-qty-input');
         const addRow = wrapper.find('.shop-single-add-row');
         ...
         if (qty > 0) {
             ...
             addRow.show();
             btn.show();
             addRow.find('.shop-single-local-qty .quantity').val(1);
         }
     }

     function updateProductCartQuantity(productId, quantity, wrapper) {
         return $.ajax({
             url: "{{ route('frontend.updateCartQuantity') }}",
             type: "POST",
             headers: {
                 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
             },
             ...
             if (response.status) {
                 syncProductCartAction(wrapper, response.quantity_in_cart);
                 viewCart();
             } else {
                 Toast.fire({
                     icon: 'warning',
                     title: response.message || 'Unable to update cart.'
                 });
             }
         }).fail(function(xhr) {
             if (handleFrontendLoginRequired(xhr)) {
     ...
     $(document).on('click', '.product-cart-btn', function() {
         ...
         let product_id = $(this).data('product-id');
         let btn = $(this);
         let wrapper = btn.closest('.product-cart-action');
         let quantity = 1;

         // Single product page quantity field (if present)
         const singleQty = btn.closest('.shop-single-info, .shop-single-action').find('.shop-cart-qty .quantity').val();
         if (singleQty) {
             quantity = parseInt(singleQty, 10) || 1;
         }
         ...
         $.ajax({
             ...
             success: function(response) {
                 ...
                 if (response.status) {
                     ...
                     if (wrapper.length) {
                         syncProductCartAction(wrapper, response.quantity_in_cart || quantity);
                     }
                     viewCart();
                 } else {
                     Toast.fire({
     ...
     $(document).on('click', '.product-qty-plus', function(e) {
         ...
         const wrapper = $(this).closest('.product-cart-action');
         const productId = wrapper.data('product-id');
         const currentQty = parseInt(wrapper.find('.product-qty-input').val(), 10) || 0;
         updateProductCartQuantity(productId, currentQty + 1, wrapper);
     });
     ...
     $(document).on('click', '.product-qty-minus', function(e) {
         ...
         const productId = wrapper.data('product-id');
         const currentQty = parseInt(wrapper.find('.product-qty-input').val(), 10) || 0;
         updateProductCartQuantity(productId, Math.max(0, currentQty - 1), wrapper);
     });
