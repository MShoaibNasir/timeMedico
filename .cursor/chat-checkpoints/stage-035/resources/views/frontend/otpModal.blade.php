...
<script>
    ...
    $(document).off('click.otpResend', '.r_otp').on('click.otpResend', '.r_otp', function(e) {
        e.preventDefault();

        var $link = $(this);
        if ($link.data('sending')) {
            return;
        }

        $link.data('sending', true);

        $.ajax({
            url: "{{ route('frontend.resendotp') }}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
            ...
            error: function(xhr) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: xhr.responseJSON?.message || 'Unable to resend OTP. Please try again.'
                });
            },
            complete: function() {
                $link.data('sending', false);
            }
        });
    });
</script>