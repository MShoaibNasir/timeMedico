...
<!-- shop checkout -->
        <div class="shop-checkout pt-3">
            <div class="container">
				...
				{{ html()->hidden('total_amount')->value($summary['order_total']) }}
				{{ html()->hidden('delivery_charges')->value($summary['delivery_fee']) }}
				{{ html()->hidden('discount')->value($summary['product_discount']) }}
				{{ html()->hidden('after_discount_amount')->value($summary['after_discount']) }}
...
@endforeach
				...
				{{-- html()->hidden('items')->value($cartItems ? json_encode($cartItems) : '[]') --}}
...
@else
{{ html()->select('address', $customer_address)->class('form-control')->placeholder('Select Address')->required()->value(old('address')) }}
@endif
